-- Read-only post-apply verification for 20260907_p0_event_facts_batch_01.sql.
...
begin
  ...
  where migration_key='20260907_p0_event_facts_batch_01' and entity_table='batch_manifest' and entity_pk='batch';
  if manifest is null or jsonb_array_length(manifest -> 'targets') <> 3
     ...
     or exists (select 1 from private.event_data_workflow_backup where migration_key='20260907_p0_event_facts_batch_01_rollback')
     or (select count(*) from private.event_data_workflow_backup where migration_key='20260907_p0_event_facts_batch_01') <> 13 then
  ...
  if exists (
    ...
    left join private.event_data_workflow_backup eb on eb.migration_key='20260907_p0_event_facts_batch_01' and eb.entity_table='events_post_state' and eb.entity_pk=t.event_id::text
    left join private.event_data_workflow_backup db on db.migration_key='20260907_p0_event_facts_batch_01' and db.entity_table='event_editions_post_state' and db.entity_pk=t.edition_id::text
...
select e.id as event_id, d.id as edition_id, e.city, e.address, e.latitude, e.longitude,
  d.race_formats, d.registration_status, d.registration_url, d.verification_status,
  d.needs_review, d.last_verified_source_id, d.last_verified_at, d.data_confidence,
  d.legacy_event_key, e.slug, d.edition_slug
...
where migration_key='20260907_p0_event_facts_batch_01' group by entity_table order by entity_table;
rollback;
