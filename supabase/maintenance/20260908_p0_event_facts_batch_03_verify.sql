-- Read-only post-apply verification for 20260908_p0_event_facts_batch_03.sql.
...
begin
  ...
  where migration_key='20260908_p0_event_facts_batch_03' and entity_table='batch_manifest' and entity_pk='batch';
  if manifest is null or jsonb_array_length(manifest -> 'targets') <> 7
     or manifest ->> 'factual_changes_only' <> 'true' or manifest ->> 'freshness_attested' <> 'false'
     or exists (select 1 from private.event_data_workflow_backup where migration_key='20260908_p0_event_facts_batch_03_rollback')
     or (select count(*) from private.event_data_workflow_backup where migration_key='20260908_p0_event_facts_batch_03') <> 29 then
  ...
  if exists (
    ...
    left join private.event_data_workflow_backup eb on eb.migration_key='20260908_p0_event_facts_batch_03' and eb.entity_table='events_post_state' and eb.entity_pk=t.event_id::text
    left join private.event_data_workflow_backup db on db.migration_key='20260908_p0_event_facts_batch_03' and db.entity_table='event_editions_post_state' and db.entity_pk=t.edition_id::text
...
where d.id in ('c5b4f709-a712-48cf-97b3-8daf5fb0dc7f'::uuid, '67356880-3ca8-4f72-836c-c48a92370ae0'::uuid, 'dfb09974-1570-4dfc-8ace-aa103560eabd'::uuid, 'eeb533bf-d8da-461e-b09b-d479ff8a30ab'::uuid, '02dfa931-395b-466e-8a04-720f29af0963'::uuid, '3800430f-3737-45bd-828d-dfa8cffd8716'::uuid, '549ea3d0-a669-416a-8766-10b446a8ff26'::uuid);
select entity_table, count(*) as snapshots from private.event_data_workflow_backup
where migration_key='20260908_p0_event_facts_batch_03' group by entity_table order by entity_table;
rollback;
