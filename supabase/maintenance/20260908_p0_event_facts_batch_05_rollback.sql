-- Targeted factual rollback of 20260908_p0_event_facts_batch_05.sql.
...
select set_config('app.change_reason', $reason$Rollback of P0 facts batch 05 (2026-09-08): restore reviewed prior facts only; retain needs_review, no freshness attestation.$reason$, true);
...
where b.migration_key = '20260908_p0_event_facts_batch_05' and b.entity_table = 'batch_manifest' and b.entity_pk = 'batch';
...
begin
  ...
  if exists (select 1 from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05_rollback') then
    raise exception 'P0 facts rollback already recorded';
  end if;
  if (select count(*) from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05') <> 9
     or (select count(*) from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05' and entity_table = 'events') <> 2
     or (select count(*) from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05' and entity_table = 'event_editions') <> 2
     or (select count(*) from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05' and entity_table = 'events_post_state') <> 2
     or (select count(*) from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05' and entity_table = 'event_editions_post_state') <> 2 then
  ...
  if exists (
    select 1 from p0_fact_targets t
    left join private.event_data_workflow_backup b on b.migration_key = '20260908_p0_event_facts_batch_05' and b.entity_table = 'events_post_state' and b.entity_pk = t.event_id::text
    left join public.events e on e.id = t.event_id
    where b.entity_pk is null or e.id is null or (to_jsonb(e) - 'updated_at') is distinct from (b.row_data - 'updated_at')
  ) or exists (
    select 1 from p0_fact_targets t
    left join private.event_data_workflow_backup b on b.migration_key = '20260908_p0_event_facts_batch_05' and b.entity_table = 'event_editions_post_state' and b.entity_pk = t.edition_id::text
    left join public.event_editions e on e.id = t.edition_id
    where b.entity_pk is null or e.id is null or (to_jsonb(e) - 'updated_at') is distinct from (b.row_data - 'updated_at')
  ) or exists (
    select 1 from p0_fact_targets t
    left join private.event_data_workflow_backup e on e.migration_key = '20260908_p0_event_facts_batch_05' and e.entity_table = 'events' and e.entity_pk = t.event_id::text
    left join private.event_data_workflow_backup d on d.migration_key = '20260908_p0_event_facts_batch_05' and d.entity_table = 'event_editions' and d.entity_pk = t.edition_id::text
...
select '20260908_p0_event_facts_batch_05_rollback', 'events', e.id::text, to_jsonb(e) from public.events e join p0_fact_targets t on e.id=t.event_id
union all
select '20260908_p0_event_facts_batch_05_rollback', 'event_editions', e.id::text, to_jsonb(e) from public.event_editions e join p0_fact_targets t on e.id=t.edition_id;
...
insert into private.event_data_workflow_backup (migration_key,entity_table,entity_pk,row_data)
select '20260908_p0_event_facts_batch_05_rollback', 'events_post_state', e.id::text, to_jsonb(e) from public.events e join p0_fact_targets t on e.id=t.event_id
union all
select '20260908_p0_event_facts_batch_05_rollback', 'event_editions_post_state', e.id::text, to_jsonb(e) from public.event_editions e join p0_fact_targets t on e.id=t.edition_id;
insert into private.event_data_workflow_backup (migration_key,entity_table,entity_pk,row_data)
select '20260908_p0_event_facts_batch_05_rollback', 'batch_manifest', 'batch', jsonb_build_object(
  'invariants',value,'actor',current_user,'auth_uid',auth.uid(),'reason',$reason$Rollback of P0 facts batch 05 (2026-09-08): restore reviewed prior facts only; retain needs_review, no freshness attestation.$reason$,
  'factual_changes_only',true,'freshness_attested',false) from p0_fact_invariants;
do $backup_guard$
begin
  if (select count(*) from private.event_data_workflow_backup where migration_key = '20260908_p0_event_facts_batch_05_rollback') <> 9 then
    raise exception 'P0 facts rollback snapshots incomplete: expected 4 before, 4 after and 1 manifest';
  end if;
end
$backup_guard$;
commit;
