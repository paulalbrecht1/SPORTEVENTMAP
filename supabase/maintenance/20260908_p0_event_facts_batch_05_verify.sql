-- Read-only post-apply verification for 20260908_p0_event_facts_batch_05.sql.
...
begin
  ...
  where migration_key='20260908_p0_event_facts_batch_05' and entity_table='batch_manifest' and entity_pk='batch';
  if manifest is null or jsonb_array_length(manifest -> 'targets') <> 2
     or manifest ->> 'factual_changes_only' <> 'true' or manifest ->> 'freshness_attested' <> 'false'
     or exists (select 1 from private.event_data_workflow_backup where migration_key='20260908_p0_event_facts_batch_05_rollback')
     or (select count(*) from private.event_data_workflow_backup where migration_key='20260908_p0_event_facts_batch_05') <> 9 then
    raise exception 'P0 facts verification: committed apply snapshots absent/incomplete or rollback present';
  end if;
  if (select array_agg(t.event_id order by t.event_id) from jsonb_to_recordset(manifest -> 'targets') as t(event_id bigint)) is distinct from array[159,342]::bigint[]
     or (select count(*) from public.public_event_discovery where event_id in (159,342)) <> 2
     or exists (select 1 from public.public_event_discovery v join public.event_editions d on d.id=v.edition_id where v.event_id in (159,342) and v.race_formats is distinct from d.race_formats) then
  ...
  if exists (
    ...
    left join private.event_data_workflow_backup eb on eb.migration_key='20260908_p0_event_facts_batch_05' and eb.entity_table='events_post_state' and eb.entity_pk=t.event_id::text
    left join private.event_data_workflow_backup db on db.migration_key='20260908_p0_event_facts_batch_05' and db.entity_table='event_editions_post_state' and db.entity_pk=t.edition_id::text
...
where d.id in ('835704af-8956-474b-8d3c-0fd44415c8b0'::uuid, '15924c4f-c20d-4419-8550-2ca397c5abb7'::uuid);
select entity_table, count(*) as snapshots from private.event_data_workflow_backup
where migration_key='20260908_p0_event_facts_batch_05' group by entity_table order by entity_table;
rollback;
