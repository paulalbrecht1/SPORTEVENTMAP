-- Emergency rollback of the public structured-format projection.
...
-- Keep the race_formats column and its type/position for existing dependencies.
-- To restore use 20260908_public_race_formats_contract_restore.sql, then postflight.
begin;
...
begin
  foreach target_name in array array['public_event_discovery', 'public_event_archive'] loop
    ...
    if mismatch then raise exception 'Rollback requires the exact active format projection: %', target_name; end if;

    select jsonb_agg(jsonb_build_array(attname, atttypid, atttypmod, attnum) order by attnum),
      string_agg(case when attname = 'race_formats' then 'NULL::jsonb as race_formats'
