-- Existing-event factual correction generated from an explicitly reviewed manifest.
...
select * from jsonb_to_recordset(@@TARGETS_JSON@@::jsonb) as t(event_id bigint, edition_id uuid, source_id uuid, source_url text, before_event jsonb, before_edition jsonb, event_patch jsonb, edition_patch jsonb);
...
do $preflight$
begin
  -- Recheck live blockers after source/event/edition locks, before any facts.
  -- The normal blocker triggers take the same parent lock; SERIALIZABLE also
  -- protects the read predicates against competing writes. Never auto-resolve.
  if exists (
  ...
  if now() < timestamptz '@@PREPARED_AT@@'
     or now() >= timestamptz '@@EXPIRES_AT@@' then
    raise exception 'P0 facts evidence window expired or not yet valid; refresh official source review';
  end if;
  if exists (select 1 from private.event_data_workflow_backup where migration_key in ('@@BATCH_KEY@@', '@@BATCH_KEY@@_rollback')) then
  ...
  -- Exact source and sibling snapshots supplement the full event/edition guard.
  if exists (
    select 1 from jsonb_to_recordset(@@BASELINE_ROWS_JSON@@::jsonb)
      as b(event_id bigint, sources jsonb, sibling_editions jsonb)
    where (select coalesce(jsonb_agg(to_jsonb(s)-'updated_at' order by s.id), '[]'::jsonb)
           from public.event_sources s where s.event_id=b.event_id)
       is distinct from (select coalesce(jsonb_agg(x-'updated_at' order by x->>'id'), '[]'::jsonb) from jsonb_array_elements(b.sources) x)
       or (select coalesce(jsonb_agg(to_jsonb(d)-'updated_at' order by d.id), '[]'::jsonb)
           from public.event_editions d where d.event_id=b.event_id)
       is distinct from (select coalesce(jsonb_agg(x-'updated_at' order by x->>'id'), '[]'::jsonb) from jsonb_array_elements(b.sibling_editions) x)
  ) then
    raise exception 'P0 facts reviewed source or sibling snapshot drifted';
  end if;
  ...
  if exists (
    ...
    where e.event_id in (@@EVENT_IDS@@) and e.publication_status = 'published'
...
select '@@BATCH_KEY@@', 'batch_manifest', 'batch',
  jsonb_build_object('targets',(select jsonb_agg(to_jsonb(t) order by event_id) from p0_fact_targets t),
    ...
    'package_provenance',@@PROVENANCE_JSON@@::jsonb)
...
insert into private.event_data_workflow_backup (migration_key,entity_table,entity_pk,row_data)
select '@@BATCH_KEY@@', 'events_post_state', e.id::text, to_jsonb(e)
from public.events e join p0_fact_targets t on t.event_id = e.id
union all
select '@@BATCH_KEY@@', 'event_editions_post_state', e.id::text, to_jsonb(e)
from public.event_editions e join p0_fact_targets t on t.edition_id = e.id;

do $backup_guard$
begin
  if (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@') <> @@SNAPSHOT_COUNT@@ then
    raise exception 'P0 facts snapshots incomplete: expected @@BEFORE_COUNT@@ before, @@BEFORE_COUNT@@ after and 1 manifest';
  end if;
end
$backup_guard$;
commit;
