-- Targeted factual rollback of @@BATCH_KEY@@.sql.
...
select pg_advisory_xact_lock(hashtextextended('sporteventmap:@@BATCH_KEY@@', 0));
...
where b.migration_key = '@@BATCH_KEY@@' and b.entity_table = 'batch_manifest' and b.entity_pk = 'batch';
...
select 1 from public.event_sources s where s.event_id in (@@EVENT_IDS@@)
order by s.id for update;
select 1 from public.events e where e.id in (@@EVENT_IDS@@)
order by e.id for update;
select 1 from public.event_editions e where e.event_id in (@@EVENT_IDS@@)
...
begin
  ...
  if exists (
    ...
    where jsonb_typeof(t.event_patch) is distinct from 'object'
      ...
      or exists (select 1 from jsonb_object_keys(t.event_patch) field where field not in ('address','canonical_name','city','country','description','distance','event_url','latitude','longitude','registration_status','sport'))
      ...
      or jsonb_array_length((t.before_edition || t.edition_patch) -> 'race_formats') <> @@PROGRAM_CASE@@
  ...
  if exists (select 1 from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@_rollback') then
    raise exception 'P0 facts rollback already recorded';
  end if;
  if (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@') <> @@SNAPSHOT_COUNT@@
     or (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@' and entity_table = 'events') <> @@COUNT@@
     or (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@' and entity_table = 'event_editions') <> @@COUNT@@
     or (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@' and entity_table = 'events_post_state') <> @@COUNT@@
     or (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@' and entity_table = 'event_editions_post_state') <> @@COUNT@@ then
    raise exception 'P0 facts rollback requires complete before/after snapshots';
  end if;
  if (select count(*) from p0_fact_targets) <> @@COUNT@@
     or (select array_agg(event_id order by event_id) from p0_fact_targets) <> array[@@EVENT_IDS@@]::bigint[]
     or exists (
       select 1 from p0_fact_targets t
  ...
  if exists (
    select 1 from p0_fact_targets t
    left join private.event_data_workflow_backup b on b.migration_key = '@@BATCH_KEY@@' and b.entity_table = 'events_post_state' and b.entity_pk = t.event_id::text
    left join public.events e on e.id = t.event_id
    where b.entity_pk is null or e.id is null or (to_jsonb(e) - 'updated_at') is distinct from (b.row_data - 'updated_at')
  ) or exists (
    select 1 from p0_fact_targets t
    left join private.event_data_workflow_backup b on b.migration_key = '@@BATCH_KEY@@' and b.entity_table = 'event_editions_post_state' and b.entity_pk = t.edition_id::text
    left join public.event_editions e on e.id = t.edition_id
    where b.entity_pk is null or e.id is null or (to_jsonb(e) - 'updated_at') is distinct from (b.row_data - 'updated_at')
  ) or exists (
    select 1 from p0_fact_targets t
    left join private.event_data_workflow_backup e on e.migration_key = '@@BATCH_KEY@@' and e.entity_table = 'events' and e.entity_pk = t.event_id::text
    left join private.event_data_workflow_backup d on d.migration_key = '@@BATCH_KEY@@' and d.entity_table = 'event_editions' and d.entity_pk = t.edition_id::text
...
create temporary table p0_fact_invariants on commit drop as select jsonb_build_object(
    ...
    'other_events_digest', (select md5(coalesce(jsonb_agg(to_jsonb(e) order by e.id)::text, '[]')) from public.events e where e.id not in (@@EVENT_IDS@@)),
    'other_editions_digest', (select md5(coalesce(jsonb_agg(to_jsonb(e) order by e.id)::text, '[]')) from public.event_editions e where e.id not in (@@EDITION_IDS@@)),
    'target_sources', (select coalesce(jsonb_agg(to_jsonb(s) order by s.id), '[]'::jsonb) from public.event_sources s where s.event_id in (@@EVENT_IDS@@))
  ) as value;
insert into private.event_data_workflow_backup (migration_key,entity_table,entity_pk,row_data)
select '@@BATCH_KEY@@_rollback', 'events', e.id::text, to_jsonb(e) from public.events e join p0_fact_targets t on e.id=t.event_id
union all
select '@@BATCH_KEY@@_rollback', 'event_editions', e.id::text, to_jsonb(e) from public.event_editions e join p0_fact_targets t on e.id=t.edition_id;
...
do $postflight$
begin
  if (select count(*) from public.events where id in (@@EVENT_IDS@@)) <> @@COUNT@@
     or (select count(*) from public.event_editions e join p0_fact_targets t on e.id = t.edition_id) <> @@COUNT@@
     or exists (
...
insert into private.event_data_workflow_backup (migration_key,entity_table,entity_pk,row_data)
select '@@BATCH_KEY@@_rollback', 'events_post_state', e.id::text, to_jsonb(e) from public.events e join p0_fact_targets t on e.id=t.event_id
union all
select '@@BATCH_KEY@@_rollback', 'event_editions_post_state', e.id::text, to_jsonb(e) from public.event_editions e join p0_fact_targets t on e.id=t.edition_id;
insert into private.event_data_workflow_backup (migration_key,entity_table,entity_pk,row_data)
select '@@BATCH_KEY@@_rollback', 'batch_manifest', 'batch', jsonb_build_object(
...
do $backup_guard$
begin
  if (select count(*) from private.event_data_workflow_backup where migration_key = '@@BATCH_KEY@@_rollback') <> @@SNAPSHOT_COUNT@@ then
    raise exception 'P0 facts rollback snapshots incomplete: expected @@BEFORE_COUNT@@ before, @@BEFORE_COUNT@@ after and 1 manifest';
  end if;
end
$backup_guard$;
commit;
